          ...
          {roles.map((role, index) => (
            ...
            >
              <div style={{
                ...
                <role.icon size={28} stroke={colors.surface} />
              </div>
              <div style={{ flex: 1 }}>
...
export function ClientBottomNav({ active }: { active: string }) {
  ...
  const navItems = [
    ...
    { id: 'calendar', label: 'Calendar', icon: Calendar, path: '/client/calendar' },
  ...
  return (
    ...
    >
      <div style={{
        ...
        <Icon size={32} stroke={colors.surface} />
      </div>
...
export function BusinessCard({ 
  ...
  business: { id: string; name: string; address: string; rating: number; reviewCount: number; description: string };
  onClick: () => void;
}) {
  return (
    <motion.div
    ...
    >
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        <div style={{ flex: 1 }}>
          <h3 style={{ fontSize: 16, fontWeight: 600, marginBottom: 8, color: colors.textPrimary }}>
            {business.name}
          </h3>
          <div style={{ display: 'flex', alignItems: 'center', gap: 4, marginBottom: 8 }}>
            <Star size={14} fill={colors.accent} stroke={colors.accent} />
            <span style={{ fontSize: 14, fontWeight: 600 }}>{business.rating}</span>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
            ...
            <span style={{ fontSize: 12, color: colors.textMuted }}>{business.address}</span>
          </div>
        </div>
        <ChevronRight size={20} stroke={colors.textMuted} />
      </div>
...
export function TimeSlotButton({ 
  ...
  slot: { id: string; startTime: string; endTime: string; available: boolean };
  selected: boolean;
  onClick: () => void;
}) {
  return (
    <button
      className={`time-slot ${selected ? 'selected' : ''} ${!slot.available ? 'disabled' : ''}`}
      ...
      style={{
        ...
        border: slot.available ? (selected ? colors.primary : colors.secondary) : colors.secondary,
...
export function ReviewCard({ 
  ...
  review: { userName: string; rating: number; comment: string; createdAt: Date };
}) {
  return (
    <div style={{
      background: colors.surface,
      ...
      <div style={{ display: 'flex', justifyContent: 'space-between', marginBottom: 8 }}>
        <span style={{ fontWeight: 600, color: colors.textPrimary }}>{review.userName}</span>
        <div style={{ display: 'flex', gap: 2 }}>
          {[1,2,3,4,5].map((star) => (
            <Star key={star} size={14} fill={star <= review.rating ? colors.accent : 'transparent'} stroke={colors.accent} />
          ))}
        </div>
      </div>
...
export function Button({ 
  ...
  return (
    <motion.button
      type={type}
      className={variant === 'primary' ? 'btn-primary' : variant === 'secondary' ? 'btn-secondary' : ''}
      onClick={onClick}
      disabled={disabled}
...
export function Input({ 
  ...
  return (
    <div style={{ marginBottom: 16 }}>
      {label && <label style={{ display: 'block', marginBottom: 8, fontWeight: 500, color: colors.textPrimary }}>{label}</label>}
      <input
        className="input"
        type={type}
        value={value}
  ...
  return (
    <div
      ...
      style={{
        padding: '8px 16px',
        borderRadius: 20,
        background: selected ? colors.primary : colors.secondary,
        color: selected ? colors.surface : colors.textPrimary,
        fontWeight: 500,
        fontSize: 14,
        cursor: 'pointer',
...
export function Badge({ 
  ...
  const colors_map = {
    success: { bg: 'rgba(124, 185, 139, 0.15)', color: colors.success },
    warning: { bg: 'rgba(255, 183, 77, 0.15)', color: colors.warning },
    error: { bg: 'rgba(229, 115, 115, 0.15)', color: colors.error },
  };
  
  return (
    <span className="badge" style={{ background: colors_map[variant].bg, color: colors_map[variant].color }}>
      {children}
    </span>
  );
}
